const afriza = async (m, { conn }) => {
    const hijabImages = global.hijab;
    const randomIndex = Math.floor(Math.random() * hijabImages.length);
    const url = hijabImages[randomIndex];

    conn.sendMessage(m.chat, {
        image: {
            url: url
        },
        caption: "Random hijab >///<",
    }, m); // Membalas pesan dengan menggunakan metode reply
};

afriza.command = /^(hijab)$/i;
afriza.tags = ['cecan'];
afriza.help = ['hijab'];

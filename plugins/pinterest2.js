...
let handler = async (m, { conn, text }) => {
    const blockedKeywords = ["p"];
    if (blockedKeywords.some(keyword => text.includes(keyword))) {
        await conn.reply(m.chat, "_Maaf fitur ini hanya untuk owner dan user premium_", m);
        return m.react('❌'); // React dengan emoji ❌ jika pesan mengandung kata yang diblokir
    }

    const count = 4; // Jumlah gambar default adalah 10
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                        body: proto.Message.InteractiveMessage.Body.fromObject({
                            text: '```Berikut Foto Yang Ditemui```',
                        }),
                        header: proto.Message.InteractiveMessage.Header.fromObject({

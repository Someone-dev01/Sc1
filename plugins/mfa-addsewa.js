...
let afriza = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  var anubot = owner[0]
  m.reply(`Wait for 3 seconds the bot will join`)
  await delay(3000)
  try {
  ...
  if (e.length) await m.reply(`Success Invite Bot To Group\n*Name Group:* ${await conn.getName(res)}\n*Expired On:* ${msToDate(global.db.data.chats[res].expired - now)}`)
...
Terimakasih telah mengundang *${m.name}*`, fkonn, {
    mentions: d
     ...
     if (!e.length) await m.reply(`Success Invite Bot To Group\n*Name Group:* ${await conn.getName(res)}\n*Expired On:* ${msToDate(global.db.data.chats[res].expired - now)}`).then(async () => {
...
afriza.help = ['addsewa *<link> <day>*']
afriza.tags = ['owner']
afriza.command = /^adds(ewa)?$/i

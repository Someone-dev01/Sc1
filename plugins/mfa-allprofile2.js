...
Nomor: ${PhoneNumber('+' + who.replace('@s.whatsapp.net', '')).getNumber('internasional')}
Link: https://wa.me/${who.split`@`[0]}${registered ? '\nUsia: ' + age : ''}
Terdaftar: ${registered ? 'Ya (sejak ' + new Date(regTime).toLocaleDateString('id-ID') + ')': 'Tidak'}
Premium: ${premium ? 'Ya' : 'Tidak'}
Kadaluarsa Premium: ${(premiumDate - now) > 1 ? msToDate(premiumDate - now) : '*Tidak ada masa berlaku premium yang ditetapkan!*'}${lastclaim > 0 ? '\nPengklaiman Terakhir: ' + new Date(lastclaim).toLocaleString('id-ID') : ''}
...
afriza.help = ['allprofile <@user>']
afriza.tags = ['info']
afriza.command = /^allprofile2$/i
afriza.limit = true

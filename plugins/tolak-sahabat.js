let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      conn.reply(m.chat,`*Maaf @${user.split('@')[0]} ga ngajak lu sahabatan kocak*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }else{
      global.db.data.users[user].pasangan = ""
      conn.reply(m.chat,`*Anda baru saja menolak ajakan persahabatan dengan @${user.split('@')[0]} Jiah, Kasian ga di anggep :)*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
}
handler.help = ['tolak-s *@tag*']
handler.tags = ['fun']
handler.command = /^(tolak-s)$/i
handler.mods = false
